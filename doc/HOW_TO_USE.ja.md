...
それぞれのモデルには、神がかった得意の表現、致命的な苦手構図、独特な愛おしい癖、そして思わぬ場面で奇跡を起こす瞬間が秘められておる。

しかし、そのCheckpointの評価（格付け）はこれまで、どうしても感覚的な闇に葬られがちじゃった。  
...
3. **5値の絶対格付け：** 評価ステータスは、人間の脳に負担をかけない `favorite` / `nice` / `keep` / `delete` / `none` の5択（トグル）だけでシンプルに完結させるべきじゃ。
...
6. **時空の遅延解決（Lazy Resolution）：** そのフィルタリングを常に有効にするため、Checkpointの選択はジョブ登録時（Queueボタンを押した時）ではなく、**実際に生成ラインが走る「実行直前」に遅延解決（オンデマンド解決）すべき**なのじゃ！
7. **不死身のプロトコル：** 実行時遅延解決であるならば、ComfyUIを再起動させることなくモデルを追加・削除して `Refresh All` をブチ込んでも、システムはエラーを吐かずに最新状態へ追従できるのじゃ！
8. **並列世界の検分：** 評価の執行は、バックグラウンドで画像作成ジョブを全力疾走させたまま、JavaScriptの無線イベントを用いて、別タブで過去の出力画像をニヤニヤ確認しながら完全非同期で行えるべきじゃ！
9. **無侵襲の増設（Non-Invasive Extension）：** 
   これほど強力な非同期評価システムでありながら、既存のメイン生成ラインを1ミリも汚さず、今使っているお気に入りのワークフローの `Load Checkpoint` ノードの頭に `Checkpoint Name Cycler` をただ1個カチッと挟み込むだけで、別タブへの「無線棚卸し環境」が即座に自動展開される。導入のために過去の遺産（既存の配線）を破壊させるような無粋な真似は、我がシャドウガーデンの流儀には反するのじゃよ！
